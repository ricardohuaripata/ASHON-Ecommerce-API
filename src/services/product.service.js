...
export const queryProductsBySearch = catchAsync(async (searchParam) => {
  const populateQuery = [
    { path: 'colors', select: 'color' },
    { path: 'sizes', select: 'size' }
  ];
  ...
  // If no products match the search parameter
...
export const queryProductsByIds = catchAsync(async (productIds) => {
  const populateQuery = [
    { path: 'colors', select: 'color' },
    { path: 'sizes', select: 'size' }
  ];
  ...
  // 1) Check if any products are found
  if (!products.length) {
    return {
      type: 'Error',
      message: 'noProductsFound',
  ...
  // 2) If everything is OK, send products
  return {
    type: 'Success',
    message: 'successfulProductsFound',
    statusCode: 200,
